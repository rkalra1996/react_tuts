...
const Person2 = (props) => {
    ...
    return (
        <div className="parent2">
            {/**
             * Note that passing arguments in the handler using anonymous fumction can be inefficient, it is recommended to use the binded way
             * example : the below function call can be written as incrementPerson2Age.bind(this, 0) without an anon function
             */}
            <p onClick={() => incrementPerson2Age(0)}>This is person2 with name {personState.persons[0].name} and age {personState.persons[0].age}</p>
            <p onClick={incrementPerson2Age.bind(this,1)}>This is person2 with name {personState.persons[1].name} and age {personState.persons[1].age}</p>
